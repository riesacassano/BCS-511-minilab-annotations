...
This list shows us all subjects, including those who failed the headphone check and heard 0 stimuli. The only subject with an incomplete set of data is `377660`. They are filtered out before the responses are saved. There are 18 subjects who completed all 12 stimuli. Responses are saved as strings separated by semicolons (`responses_timestamps_str.csv`).
...
# separate the response values into numerics and give them each their own row
#d.responses_all_long <- d.responses_all %>%
#  separate(response_vals, into='timestamps', sep=';') %>% # unhappy
#  print()


```

As a final processing step, we'll represent the responses as binary vectors. For this we'll bin into one-second bins. 1 represents seconds where a responses occurred in that second, and 0 represents seconds where no response occurred in that second. Since not all of the stimuli are the same length, there are variable amounts of padding added to the end of each binary vector (maximum 6 seconds). The binarized representations are saved in both wide (`responses_binarized_wide.csv`) and long (`responses_binarized_long.csv`) forms.
