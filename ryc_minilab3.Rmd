...
#compute_bootstrap_isc <- function(){
  # compute true ISC
  
  # perform n permutations (shuffle=TRUE)
  
#}

compute_isc <- function(data, stim, shuffle=FALSE){
  ...
  for (sub in seq(n_subj)){ # this works because subjects are labelled 1 to 43
  #for (sub in levels(data$exp_subject_id)){
    this_sub_data <- data %>%
...
avg_isc <- compute_isc(data_long,test_stim)
print(avg_isc)
