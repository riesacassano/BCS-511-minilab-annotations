...
```{r isc}
# specify particular subject and stimulus for testing
this_sub <- '377777'
specific_stim <- c('intact',1,2)

this_sub_data <- data_long %>%
  filter(exp_subject_id == this_sub,
         scram_cond == specific_stim[1],
         stim_num == specific_stim[2],
         run == specific_stim[3]) %>%
  print()

avg_others_data <- data_long %>%
  filter(exp_subject_id != this_sub,
         scram_cond == specific_stim[1],
         stim_num == specific_stim[2],
         run == specific_stim[3]) %>%
...
print(cor(this_sub_data$response,avg_others_data$avg_resp))
